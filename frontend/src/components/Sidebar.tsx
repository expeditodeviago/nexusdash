...
export const Sidebar: React.FC<SidebarProps> = ({ 
  data, activeCharts, activeFilters, toggleChart, setActiveFilters, onGoHome, logo, setLogo 
}) => {
  ...
  return (
    <aside className="w-[320px] bg-white dark:bg-[#0a0e1a] border-r border-slate-200 dark:border-slate-800 p-8 flex flex-col z-30 overflow-y-auto transition-colors duration-300">
      <div className="flex items-center gap-4 mb-10 cursor-pointer group" onClick={onGoHome}>
      ...
      {/* IDENTIDADE VISUAL */}
      <div className="mb-10 p-4 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-[1.5rem]">
        ...
        {logo ? (
          ...
          <label className="flex flex-col items-center justify-center py-6 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl cursor-pointer hover:border-cyan-500 hover:bg-cyan-500/5 transition-all group">
            ...
            <span className="text-[10px] font-bold text-slate-400 group-hover:text-cyan-600 uppercase">Carregar Logo</span>
